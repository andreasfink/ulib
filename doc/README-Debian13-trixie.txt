...
reference counting because its using the old objc legacy runtime which does not support
ARC. Hence ulib can not run with the legacy runtime and requires the whole GNUStep 
libraries to be compiled with the new runtime


Here is how to get such a installation up and running under Debian 13 (codename Trixie)
...
#   Note libiconv does not build if the compiler is set to clang or the linker to lld.
    ...
    cd libiconv-1.18
    CC=gcc CXX="gcc++" CFLAGS="-fPIC" CPPFLAGS="-fPIC" ./configure --enable-static --enable-dynamic
    make
    make install
